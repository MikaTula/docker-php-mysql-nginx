...
2. Контроллер API + при необходимости `FormRequest` и политики доступа. В методах вида `update(...UpdateRequest $request, $entityId)` (когда второй аргумент — это просто `id` без model-binding) использовать параметр названия `$id`, а не `$entity`.
